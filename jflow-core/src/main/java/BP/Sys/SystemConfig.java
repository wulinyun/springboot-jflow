...
public class SystemConfig {
	...
	public static String getPathOfWebApp() {
		// return "D:\\JJFlow\\trunk\\JJFlow";
		Object pathOfWebApp = SystemConfig.getAppSettings().get("PathOfWebApp");
		if (SystemConfig.getIsBSsystem()) {
			if (Glo.getRequest() == null || Glo.getRequest().getSession() == null) {
				return BP.WF.Glo.getHostURL() + "/";
			} else {
				//为本地调试环境,直接返回配置的数据目录
				if( pathOfWebApp!=null&&!"".equals(pathOfWebApp)){
					System.out.println("pathOfWebApp"+pathOfWebApp);
					return pathOfWebApp.toString();
				}else{
